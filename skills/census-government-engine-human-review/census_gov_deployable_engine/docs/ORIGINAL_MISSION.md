...
Original mission correction: CENSUS is complete only when raw history can cross into verified canon through an operator-controlled review node. The engine therefore includes `review_canon.py` / `python -m census_engine.cli review` as the practical bridge from extracted candidate to reviewed active ledger.
